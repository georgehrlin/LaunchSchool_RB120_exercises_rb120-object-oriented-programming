=begin
Once I ran the faulty code, it raised a NoMethodError for line 56 with an error
message of `==': private method `serial_number' called for
#<Employee:0x00000001011d0568 @name="Raul", @serial_number="399-1007-4242">.

This error stems from the definition of Employee#==, which calls == on the
serial_number of the current object with other.serial_number as a argument.
other.serial_number runs into an error because Employee#.serial_number is a
private instance method, which makes it exclusive to the current object and
cannot be called on a different object even if this other object is of the same
class.

To fix this issue, I will simply put attr_reader :serial_number under protected
instead of private.
=end
